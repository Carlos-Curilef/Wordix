...
$opcionSubMenu = 0;
    ...
    switch ($opcion){
        case 1: // Jugar al wordix con una palabra elegida
            ...
            for ($i=0;$i<count($partidasCargadas);$i++){ // se verifica de que el jugador no juegue con una palabra que ya utilizó
                if ($partidasCargadas[$i]["jugador"] == $nombre && $coleccionPalabras[$valor-1] == $partidasCargadas[$i]["palabraWordix"]){
                        ...
                        $i=-1; // se da este valor si es que se vuelve a ingresar el mismo número de palabra
                     }
            }
        ...
        case 2: // Jugar al wordix con una palabra aleatoria, se usó la misma lógica que el caso 1, solo cambia que la elección la realiza la función aleatoria "rand()"
            ...
            for ($i=0;$i<count($partidasCargadas);$i++){ // se verifica de que el jugador no juegue con una palabra que ya utilizó
                if ($partidasCargadas[$i]["jugador"] == $nombre && $coleccionPalabras[$valor-1] == $partidasCargadas[$i]["palabraWordix"]){
                        $valor=rand(1, $numPalabras); // se solicita otro valor si es ese el caso
                        $i=-1; // se da este valor si es que se vuelve a ingresar el mismo número de palabra
                     }
            }
        ...
        case 3: // Mostrar una partida
            ...
            echo datosPar($partidasCargadas, solicitarNumero(1, $cont)-1); // se solicita al usuario un número válido de partida, se resta -1 para tener el indice y se muestran los datos
            break;
